...
<div class="wrapper">
    ...
    <div class="container">
        ...
        <c:forEach var="music" items="${musics.result}">
            <c:if test="${music.isdeleted==0}">
                <div class="jumbotron">
                    <div class="media">
                        <div class="media-left">
                            <a data-toggle="modal" data-target="#myModal" data-whatever="${music.id}">
...
<script>
    ...
    $().ready(function (){
        ...
        xhr.onload = function () {
            ...
            musics.forEach(music => {
                if (!years.includes(music.publishyear)) {
                    years.push(music.publishyear)
                }
                if (!types.includes(music.type)) {
                    types.push(music.type)
                }
            })
            years.sort((a, b) => a - b)
            // 使用jquery的id选择器动态添加years与musics到下拉菜单
            ...
            $('#selectYearAndTypeBar').find('li').each(function () {
                ...
                if(currentLocation === '/user/musics/AAA'
                    || currentLocation === '/user/musics/AAA'
                ){
    ...
    $(document).ready(function () {
        $("#submit").click(function () {
            const message = {
                ...
                "objectid": currentMusicId,
                "objecttype": "music", // 这个确实可以写死
    ...
    $('#myModal').on('show.bs.modal', async function (event) {
        ...
        const musicId = a.data('whatever') // 解析出data-whatever内容
        currentMusicId = musicId
        const modal = $(this)
        let flag = false;
        for (let i = 0; i < assessments.length; i++){
            if (assessments[i].objectid === musicId){
                flag = true
        ...
        if(!flag){
            modal.find('.modal-title').text('该音乐暂无评论')
        }
